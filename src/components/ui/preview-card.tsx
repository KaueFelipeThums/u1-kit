...
function PreviewCardArrow({ ...props }: React.ComponentProps<typeof BasePreviewCard.Arrow>) {
...
function PreviewCardContent({
  ...
  return (
    <PreviewCardPortal>
      <PreviewCardPositioner className="z-50 outline-none select-none" sideOffset={sideOffset} align={align}>
        <BasePreviewCard.Popup
          data-slot="preview-card-content"
          className={cn(
            'bg-popover text-popover-foreground overflow-auto max-h-[var(--available-height)] outline-border w-64 origin-[var(--transform-origin)] rounded-md p-4 shadow-md outline -outline-offset-1 transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0',
            className,
          )}
          {...props}
        >
