...
function PopoverHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return <div data-slot="popover-header" className={cn('relative flex flex-col gap-y-1', className)} {...props} />;
}

function PopoverTitle({ className, ...props }: React.ComponentProps<typeof BasePopover.Title>) {
  return <BasePopover.Title data-slot="popover-title" className={cn('text-sm font-semibold', className)} {...props} />;
}
...
function PopoverContent({
  ...
  return (
    <PopoverPortal>
      <PopoverPositioner className="z-50 outline-none select-none" sideOffset={sideOffset} align={align}>
        <BasePopover.Popup
          data-slot="popover-content"
          className={cn(
            'bg-popover overflow-auto text-popover-foreground max-h-[var(--available-height)] outline-border w-72 origin-[var(--transform-origin)] rounded-md p-4 shadow-md outline -outline-offset-1 transition-[transform,scale,opacity] data-[ending-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:scale-95 data-[starting-style]:opacity-0',
            className,
          )}
          {...props}
        >
