...
function Menubar({ className, ...props }: React.ComponentProps<typeof BaseMenubar>) {
  return (
    <BaseMenubar
      data-slot="menubar"
      className={cn('bg-popover flex h-9 items-center gap-1 rounded-md border p-1 shadow-xs', className)}
      {...props}
    />
  );
}
...
function MenubarMenu({ ...props }: React.ComponentProps<typeof BaseMenu.Root>) {
  return <BaseMenu.Root delay={0} closeDelay={0} data-slot="menubar-menu" {...props} />;
}

function MenubarGroup({ ...props }: React.ComponentProps<typeof BaseMenu.Group>) {
...
function MenubarRadioGroup({ ...props }: React.ComponentProps<typeof BaseMenu.RadioGroup>) {
    ...
    <MenubarPortal>
      ...
      >
        <BaseMenu.Popup
          ...
          className={cn(
            'bg-popover overflow-auto max-h-[var(--available-height)] data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 text-popover-foreground data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 min-w-[12rem] origin-[var(--transform-origin)] rounded-md border p-1 shadow-md',
            className,
          )}
          {...props}
        />
...
function MenubarRadioItem({ className, children, ...props }: React.ComponentProps<typeof BaseMenu.RadioItem>) {
  return (
    <BaseMenu.RadioItem
...
function MenubarSub({ ...props }: React.ComponentProps<typeof BaseMenu.SubmenuRoot>) {
  return <BaseMenu.SubmenuRoot delay={0} closeDelay={0} data-slot="menubar-sub" {...props} />;
}
