...
function AutocompletePositioner({ ...props }: React.ComponentProps<typeof BaseAutocomplete.Positioner>) {
...
function AutocompleteInput({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.Input>) {
  return (
    <BaseAutocomplete.Input
      data-slot="autocomplete-input"
      className={cn(
        'placeholder:text-muted-foreground selection:bg-primary dark:bg-input/30 bg-transparent group-hover:border-ring/70 selection:text-primary-foreground flex h-9 w-full min-w-0 rounded-md border px-3 py-1 text-base shadow-xs transition-[color,box-shadow,border-color] outline-none disabled:pointer-events-none disabled:opacity-50 md:text-sm',
...
function AutocompleteContent({
  ...
  return (
    <AutocompletePortal>
      <AutocompletePositioner className="z-50 outline-none select-none" sideOffset={sideOffset}>
        <BaseAutocomplete.Popup
          data-slot="autocomplete-content"
          className={cn(
            'bg-popover text-popover-foreground max-w-[var(--anchor-width)] relative max-h-[min(var(--available-height),20rem)] min-w-[var(--anchor-width)] origin-[var(--transform-origin)] overflow-x-hidden overflow-y-auto overscroll-contain rounded-md border p-1 shadow-md',
            className,
          )}
          {...props}
        >
...
function AutocompleteItem({ className, children, ...props }: React.ComponentProps<typeof BaseAutocomplete.Item>) {
  return (
...
function AutocompleteEmpty({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.Empty>) {
  return (
...
function AutocompleteStatus({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.Status>) {
  return (
    <BaseAutocomplete.Status
      data-slot="autocomplete-status"
      className={cn('text-muted-foreground flex items-center gap-2 px-2 py-1.5 text-sm', className)}
      {...props}
    />
  );
}
...
function AutocompleteGroupLabel({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.GroupLabel>) {
  return (
...
function AutocompleteSeparator({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.Separator>) {
  return (
...
function AutocompleteRow({ className, ...props }: React.ComponentProps<typeof BaseAutocomplete.Row>) {
  return (
