  ...
  return (
    <SheetPortal>
      ...
      <BaseSheet.Popup
        ...
        className={cn(
          'bg-popover max-w-full text-popover-foreground fixed z-50 flex max-h-[calc(100vh-2rem)] flex-col gap-4 rounded-lg shadow-lg outline-hidden transition ease-in-out data-closed:duration-300 data-open:duration-500 scale-[calc(1-0.1*var(--nested-dialogs))]',
      ...
      >
        {children}
        <SheetClose className="text-muted-foreground absolute top-4 right-4 rounded-xs opacity-50 transition-all hover:opacity-100 disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4">
...
function SheetHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex flex-col gap-2 px-6 py-4 text-center sm:text-left', className)}
      {...props}
    />
  );
}
...
function SheetBody({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      data-ui="dialog-body"
      className={cn(
        'flex flex-1 flex-col overflow-auto px-6 py-4',
        'max-h-[calc(100dvh-var(--dialog-header-height,0px)-var(--dialog-footer-height,0px))]',
        className,
      )}
    />
  );
...
function SheetTitle({ className, ...props }: React.ComponentProps<typeof BaseSheet.Title>) {
  return (
    <BaseSheet.Title data-slot="sheet-title" className={cn('text-foreground font-semibold', className)} {...props} />
  );
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex flex-col-reverse px-6 py-4 gap-2 sm:flex-row sm:justify-end', className)}
      {...props}
    />
  );
}
